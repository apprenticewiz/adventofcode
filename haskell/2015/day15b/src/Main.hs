...
totalCalories ingredients amounts = sum [ calories ing * amt | (ing, amt) <- zip ingredients amounts ]

maxScore :: [Ingredient] -> Int
maxScore ingredients = maximum [ score ingredients alloc
                               | alloc <- distribute 100 (length ingredients)
                               , totalCalories ingredients alloc == 500
                               ]
