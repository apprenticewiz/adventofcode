module IntCode ( Cpu(memory, inputs), initCpu, execute, RunResult(..) ) where
...
executeM cpu currentOutputs = do
  ...
  where
        ...
        executeInstruction :: CpuM s -> Instruction -> [Int] -> ST s RunResult
        executeInstruction (CpuM mem pc inps idx) (operation, parameters) outputs = do
            case operation of
                        ...
                        [Position inputAddr] -> do
                            if idx < length inps
                                then do
                                    ...
                                    frozen <- Vector.freeze mem
                                    pure $ NeedInput (Cpu frozen pc inps idx) (reverse outputs)
                ...
                Halt -> pure $ Halted (reverse outputs)
