...
process content =
    ...
    in case ls of
         (wire1:wire2:_) ->
            ...
            in minimum [ calcSteps (segments1, segments2) is | is <- intersections ]
    ...
    where
        ...
        calcSteps :: ([Segment], [Segment]) -> Point -> Int
        calcSteps (segments1, segments2) point =
            let s1 = stepsToPoint segments1 point
                s2 = stepsToPoint segments2 point
            in s1 + s2
            where
                stepsToPoint :: [Segment] -> Point -> Int
                stepsToPoint segments p = go segments p 0
                    where
                        go :: [Segment] -> Point -> Int -> Int
                        go [] _ steps = steps
                        go (segment:rest) (px, py) steps =
                            let (p1, p2) = segment
                                (x1, y1) = p1
                                (x2, y2) = p2
                                segmentLength = abs (x2 - x1) + abs (y2 - y1)
                            in if x1 == x2
                                then if px == x1 && py >= min y1 y2 && py <= max y1 y2
                                        then steps + abs (py - y1)
                                        else go rest point (steps + segmentLength)
                                else if y1 == y2
                                    then if py == y1 && px >= min x1 x2 && px <= max x1 x2
                                        then steps + abs (px - x1)
                                        else go rest point (steps + segmentLength)
