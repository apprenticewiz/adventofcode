...
findMinPath :: Graph -> Char -> Set Char -> Int
findMinPath graph start visited
    | Set.size visited == Map.size (Map.fromList [ (c, ()) | (c, _) <- Map.keys graph ]) = 0
    | otherwise =
        minimum
            [ dist + findMinPath graph next (Set.insert next visited)
            | (next, dist) <- nextSteps
