...
file = do
    ...
    eof
    let augmentedFloor0 = Map.union (head floors) (Map.fromList
            [ ("elerium", (Just 0, Just 0))
            , ("dilithium", (Just 0, Just 0))
            ])
        floors' = augmentedFloor0 : tail floors
        combined = foldl' merge Map.empty floors'
