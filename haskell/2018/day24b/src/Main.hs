...
data BattleResult = ImmuneWin [Group] | InfectionWin [Group] | Stalemate [Group]
    deriving (Show)

battle :: [Group] -> BattleResult
battle = go
  where
    go gs
      | all ((== Immune) . team) gs = ImmuneWin gs
      | all ((== Infection) . team) gs = InfectionWin gs
      | otherwise =
...
applyBoost boost = map boostGroup
  where
    boostGroup grp
      | team grp == Immune = grp { attackDmg = attackDmg grp + boost }
      | otherwise = grp

findMinBoost :: [Group] -> Int
findMinBoost groups = binarySearch 0 10000
  where
    binarySearch low high
      | low >= high = low
      | otherwise =
            let mid = (low + high) `div` 2
                boosted = applyBoost mid groups
            in case battle boosted of
                ImmuneWin _ -> binarySearch low mid
...
process content =
    case runParser file 0 "" content of
        ...
        Right groups ->
            let boost = findMinBoost groups
                boosted = applyBoost boost groups
            in case battle boosted of
                ImmuneWin survivors -> sum . map units $ survivors
                _ -> error "Immune system should win with found boost"
