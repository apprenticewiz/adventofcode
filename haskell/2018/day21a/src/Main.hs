...
runProgram :: Program -> State (Cpu, Int) (Maybe Int)
        ...
        if currentPc < 0 || currentPc >= Vector.length program
            then return Nothing
                ...
                case instruction of
                    EqRR a b _ ->
                        if b == 0 then
                            return (Just (regsWithPc Vector.! a))
                        else if a == 0 then
                            return (Just (regsWithPc Vector.! b))
                        else doStep regsWithPc cpu instruction instCount
                    _ -> doStep regsWithPc cpu instruction instCount
    doStep regsWithPc cpu instruction instCount = do
        let cpu' = cpu { registers = regsWithPc }
            cpu'' = executeInstruction instruction cpu'
            newPc = (registers cpu'' ! ipReg cpu'') + 1
        put $ (cpu'' { pc = newPc }, instCount + 1)
        go
