...
runProgram :: Program -> State (Cpu, Int) Int
runProgram prog = do
    let (l, u) = bounds prog
    (cpu, muls) <- get
    if cpu ^. ip < l || cpu ^. ip > u
        then return muls
            ...
            Set (Register x) y -> do
                ...
                modify (first (setReg x yVal))
                modify (first (ip %~ (+ 1)))
                runProgram prog
            Sub (Register x) y -> do
                let yVal = resolve y cpu
                let n = getReg x cpu
                modify (first (setReg x (n - yVal)))
                modify (first (ip %~ (+ 1)))
                runProgram prog
            Mul (Register x) y -> do
                let yVal = resolve y cpu
                let n = getReg x cpu
                modify (\(cpu', muls') -> (setReg x (n * yVal) cpu', muls' + 1))
                modify (first (ip %~ (+ 1)))
                runProgram prog
            Jnz x y -> do
...
process content =
    case parse file "" content of
        ...
        Right prog -> evalState (runProgram prog) (initCpu, 0)
  where
    initCpu = Cpu 0 0 0 0 0 0 0 0 0
