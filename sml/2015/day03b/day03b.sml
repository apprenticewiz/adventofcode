...
fun process filename =
  let 
    fun f (ch, (Position.Position(santaX, santaY), roboSanta, true, positions)) =
      let
      ...
      in
        (newSanta, roboSanta, false, newPositions)
      end
    | f (ch, (santa, Position.Position(roboSantaX, roboSantaY), false, positions)) =
      let
        val newRoboSanta = case ch of
                             #"^" => Position.Position(roboSantaX, roboSantaY + 1)
                           | #"v" => Position.Position(roboSantaX, roboSantaY - 1)
                           | #"<" => Position.Position(roboSantaX - 1, roboSantaY)
                           | #">" => Position.Position(roboSantaX + 1, roboSantaY)
                           | _    => Position.Position(roboSantaX, roboSantaY)
        val newPositions = PositionSet.insert (newRoboSanta, positions)
      in
        (santa, newRoboSanta, true, newPositions)
      end
    ...
    val (_, _, _, finalPositions) =
      List.foldl f (Position.Position(0, 0), Position.Position(0, 0), true, PositionSet.new) (String.explode content)
  in
