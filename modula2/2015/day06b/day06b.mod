...
TYPE
    GridType = ARRAY [1..RowMax] OF ARRAY [1..ColMax] OF INTEGER;
...
BEGIN
    FOR Row := R1 TO R2 DO
        FOR Col := C1 TO C2 DO
            CASE Action OF
                TurnOn  : Grid[Row, Col] := Grid[Row, Col] + 1 |
                TurnOff : Grid[Row, Col] := Maximum(0, Grid[Row, Col] - 1) |
...
BEGIN
    ...
    FOR Row := 1 TO RowMax DO
        FOR Col := 1 TO ColMax DO
            Total := Total + VAL(CARDINAL, Grid[Row, Col]);
        END;
    END;
    RETURN Total
END CountGrid;
