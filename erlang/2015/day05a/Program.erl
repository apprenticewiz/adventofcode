  ...
  case file:read_file(Filename) of
    {ok, Chars} ->
      ...
      Result = lists:foldl(
        fun
          (Line, Acc) ->
            case prop1(Line) andalso prop2(Line) andalso prop3(Line) of
              true -> Acc + 1;
              false -> Acc
            end
        end,
...
prop1(Str) ->
  case re:run(Str, "[aeiou]", [global, {capture, all, list}]) of
    {match, Matches} -> length(Matches) >= 3;
    nomatch -> false
  end.
...
prop3(Str) ->
  case re:run(Str, "(ab|cd|pq|xy)", [{capture, none}]) of
    nomatch -> true;
    _ -> false
  end.
