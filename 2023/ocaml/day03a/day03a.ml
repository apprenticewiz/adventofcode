...
let check_parts number_locs part_locs =
  List.fold_left (fun result (number_loc, number) ->
    ...
    let found_adjacent = List.fold_left (fun found number_col ->
        found || List.fold_left (fun found_adj neighbor ->
          let adjacent_pos = { row = number_row + neighbor.row; col = number_col + neighbor.col } in
          found_adj || ((PositionMap.find_opt adjacent_pos part_locs) <> None)
          ) found neighbors
      ) false (List.init (number_col_last - number_col_first + 1) (fun i -> number_col_first + i))
    in if found_adjacent
        then result + (int_of_string number)
        else result
    ) 0 (PositionMap.bindings number_locs)
