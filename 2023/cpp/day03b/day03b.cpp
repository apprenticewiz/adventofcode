    ...
    for ( auto gearlocp = gear_locs.begin(); gearlocp != gear_locs.end(); gearlocp++ ) {
        ...
        for ( auto numlocp = number_locs.begin(); numlocp != number_locs.end(); numlocp++ ) {
            ...
            for ( int delta_row = -1; delta_row <= 1; delta_row++ ) {
                int adjacent_row = gearlocp->first.first + delta_row;
                for ( int delta_col = -1; delta_col <= 1; delta_col++ ) {
                    int adjacent_col = gearlocp->first.second + delta_col;
                    ...
                    for ( int number_col = number_col_start; number_col < number_col_end; number_col++ ) {
                        if ( !skip && adjacent_row == numlocp->first.first && adjacent_col == number_col ) {
                            skip = true;
                            adjacents.push_back(std::atoi(numlocp->second.c_str()));
                        }
                    }
                }
            }
        ...
        if ( adjacents.size() == 2 ) {
            result += std::accumulate(adjacents.begin(), adjacents.end(), 1, std::multiplies<int>());
        }
    }
    return result;
}
