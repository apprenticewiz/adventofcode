...
(defn perform [grid action r1 c1 r2 c2]
  (reduce
    (fn [curr-grid [row col]]
      (let [old-val (grid [row col] 0)]
        (case action
          :turn-on (assoc curr-grid [row col] (+ old-val 1))
          :turn-off (assoc curr-grid [row col] (if (> old-val 0) (- old-val 1) 0))
          :toggle (assoc curr-grid [row col] (+ old-val 2)))))
    grid
