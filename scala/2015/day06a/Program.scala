...
object Program {
  ...
  def perform(grid: Array[Boolean], action: String, r1: Int, c1: Int, r2: Int, c2: Int) = {
    for (row <- r1 to r2) {
      for (col <- c1 to c2) {
        action match {
          case "turn on" => grid(row * 1000 + col) = true
          case "turn off" => grid(row * 1000 + col) = false
          case "toggle" => grid(row * 1000 + col) = !grid(row * 1000 + col)
          case _ => ()
        }
      }
    }
  ...
  def process(filename: String): Int = {
    ...
    val grid: Array[Boolean] = Array.fill(1000*1000)(false)
