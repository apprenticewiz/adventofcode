...
fn fix_update(rules: &HashMap<u32, Vec<u32>>, update: &[u32]) -> Vec<u32> {
    let mut fixed_update = update.to_owned();
...
fn process_updates(rules: &HashMap<u32, Vec<u32>>, updates: &[Vec<u32>]) -> u32 {
    ...
    for update in updates.iter() {
        if !is_valid(rules, update) {
            ...
            let mid_idx = fixed_update.len() / 2;
            let mid_value = &fixed_update[mid_idx];
            result += mid_value;
        }
    }
    result
