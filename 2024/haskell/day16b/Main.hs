...
bestSeats :: Graph -> Node -> (Node, Node) -> Set Position
bestSeats graph startNode (endNode1, endNode2) =
    let pq = MinQueue.fromList [(0, startNode, [startNode])]
        ...
        dijkstra pq minCost bestPaths costs
            | MinQueue.null pq =
                foldl
                    (\tempSet path ->
                        let pathSet = Set.fromList $ map fst path
                        in Set.union tempSet pathSet
                    )
                    Set.empty
                    bestPaths
            | otherwise =
...
process contents =
    ...
    in Set.size (bestSeats graph (start, Horizontal) ((end, Horizontal), (end, Vertical)))
