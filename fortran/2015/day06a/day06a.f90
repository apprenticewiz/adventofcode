...
CONTAINS
  ...
  SUBROUTINE perform(grid, action, r1, c1, r2, c2)
    ...
    IF ( TRIM(action) == 'turn on' ) grid(r1:r2,c1:c2) = .TRUE.
    IF ( TRIM(action) == 'turn off' ) grid(r1:r2,c1:c2) = .FALSE.
    IF ( TRIM(action) == 'toggle' ) grid(r1:r2,c1:c2) = .NOT. grid(r1:r2,c1:c2)
