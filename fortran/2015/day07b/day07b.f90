...
CONTAINS
  ...
  INTEGER FUNCTION process(filename) RESULT(res)
    ...
    WRITE (a_str, '(I4)') a
    DO i = 1, (operations%next_free - 1)
      IF ( operations%entries(i)%key == "b" ) THEN
        operations%entries(i)%val%op = ASSIGN_OP
        operations%entries(i)%val%src1 = TRIM(a_str)
        EXIT
      END IF
    END DO
    cache%next_free = 1
