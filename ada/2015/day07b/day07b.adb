...
begin
  ...
  A := Integer(Eval(Operations, Cache, To_Unbounded_String("a")));
  Op.Operator := AssignOp;
  Op.Source1 := Trim(To_Unbounded_String(Integer'Image(A)), Side => Ada.Strings.Both);
  Operation_Maps.Replace(Operations, To_Unbounded_String("b"), Op);
  Cache := Cache_Maps.Empty_Map;
