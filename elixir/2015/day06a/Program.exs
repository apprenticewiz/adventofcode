...
defmodule Program do
  ...
  defp perform(grid, action, {r1, c1, r2, c2}) do
    Enum.reduce(for(row <- r1..r2, col <- c1..c2, do: {row, col}), grid, fn
      coord, curr_grid ->
        case action do
          "turn on" -> Map.put(curr_grid, coord, true)
          "turn off" -> Map.put(curr_grid, coord, false)
          "toggle" ->
            current = Map.get(curr_grid, coord, false)
            Map.put(curr_grid, coord, !current)
        end
    end)
  end
  ...
  defp process(filename) do
    case File.read(filename) do
      {:ok, content} ->
        ...
        total = Enum.count(grid, fn {_, x} -> x end)
        {:ok, total}
      {:error, reason} ->
        {:error, reason}
    end
