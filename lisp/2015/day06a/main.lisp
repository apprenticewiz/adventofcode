  ...
  (dotimes (i (+ (- r2 r1) 1))
    (dotimes (j (+ (- c2 c1) 1))
      (let* ((row (+ r1 i))
        ...
        (cond ((string= action "turn on") (setf (aref grid row col) t))
              ((string= action "turn off") (setf (aref grid row col) nil))
              ((string= action "toggle") (setf (aref grid row col) (not old-value))))))))

(defun count-values (grid)
  (let ((total 0))
    (dotimes (row 1000)
      (dotimes (col 1000)
        (if (aref grid row col)
          (setq total (+ 1 total)))))
          total))
